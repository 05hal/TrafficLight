...
def main() -> None:
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        print(f"epoch {epoch:03d}/{args.epochs} train_loss={train_loss:.4f} train_acc={train_acc:.3f} val_loss={val_loss:.4f} val_acc={val_acc:.3f}")
        if val_acc >= best_val_acc:
            best_val_acc = val_acc
            best_checkpoint_path = args.out.with_name(f"{args.out.stem}_epoch{epoch:03d}{args.out.suffix}")
            with best_checkpoint_path.open("wb") as f:
                torch.save(
                    {
